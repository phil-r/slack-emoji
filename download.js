...
const API_URL = `https://slack.com/api/emoji.list?token=${config.SLACK_TOKEN}`;

request({ uri: API_URL, json: true }, (err, resp, obj) => {
  downloadEmojis(obj);
});
...
function download(obj, cb) {
  ...
  request(uri).on("error", err => {
    ...
    fs.createWriteStream(`downloads/${createName(name, uri)}`)
  ).on("finish", cb);
}
